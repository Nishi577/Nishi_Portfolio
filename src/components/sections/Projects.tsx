...
const projects = [
  {
    ...
    live: "https://saamavar-f28e7.firebaseapp.com/",       // ✏️ replace
    image: "/public/Saamvara.png",                    // ✏️ replace with your image path
    accent: "var(--accent-yellow-warm)",
    tint: "var(--bg-section-3)",
  },
  {
    ...
    github: "https://github.com/Nishi577/safecity",
    live: "https://safe-city-finale.vercel.app/",        // ✏️ replace
    image: "/public/safecity.png",                    // ✏️ replace
    accent: "var(--accent-lilac)",
    tint: "var(--bg-section-2)",
  },
  {
    ...
    live: "http://bloompath-frontend-app.s3-website-us-east-1.amazonaws.com/",       // ✏️ replace
    image: "/public/bloompath.png",                   // ✏️ replace
    accent: "var(--accent-blush)",
    tint: "var(--bg-section-4)",
  },
  {
    ...
    github: "https://github.com/Nishi577/impactsphere",
    live: "https://impact-sphere-olive.vercel.app/",    // ✏️ replace
    image: "/public/impactsphere.png",                // ✏️ replace
    accent: "var(--accent-maroon-soft)",
    tint: "var(--bg-section-2)",
  },
];
...
function ProjectImage({ name, image, accent, tint }: { name: string; image: string; accent: string; tint: string }) {
  ...
  // Fallback: browser mockup if image fails to load
  return (
    <div
      className="relative w-full h-full min-h-[260px] rounded-xl overflow-hidden border"
      style={{ borderColor: "var(--border-lilac)", background: tint }}
    >
      ...
      <div className="relative p-5 h-full">
        ...
        <div className="mt-5 grid grid-cols-3 gap-2">
          {[0, 1, 2].map(i => (
            <div key={i} className="h-14 rounded-md border"
            ...
            {visible.map((p, i) => (
              ...
              >
                ...
                <div className="p-8 md:p-10 order-2 md:order-1">
                  ...
                  {/* Links — GitHub + Live (now separate) */}
                  <div className="mt-6 flex gap-5 font-mono-jb text-xs">
                ...
                {/* Preview — real image with fallback */}
                <div className="relative p-6 md:p-8 order-1 md:order-2 transition-transform duration-500 group-hover:scale-[1.02]"
                  style={{ background: p.tint }}>
                  <ProjectImage name={p.name} image={p.image} accent={p.accent} tint={p.tint} />
                </div>
              </motion.article>
            ))}
          </div>
